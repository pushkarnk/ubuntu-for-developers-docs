---
myst:
  html_meta:
    description: "Advanced Java development tutorials: GraalVM, CRaC, and devpack-for-spring on Ubuntu."
---

(java-advanced)=
# Java – advanced

These tutorials cover advanced Java development topics on Ubuntu, beyond the basic {ref}`first-program tutorial <use-java>`.

:::{toctree}
:maxdepth: 1

Devpack for Spring <devpack-for-spring>
Native compile with GraalVM <graalvm-use>
Checkpoint/Restore with OpenJDK CRaC <crac-use>
OpenJDK OpenSSL FIPS <use-openjdk-fips-openssl>
:::
